    ...
    <main class="container main-content">
      <div class="form-page">
        ...
        <div class="form-header">
          <h2 class="page-title">Edit <span>Film/Series</span></h2>
          <p class="page-sub">Update informasi film atau series kamu</p>
        </div>

        <div v-if="loadingData" class="state-msg">Memuat data...</div>
        ...
        <div v-else class="form-layout">
          ...
          <div class="preview-side">
            ...
            <p class="preview-hint">Edit URL poster untuk preview</p>
          </div>
...
async function handleSubmit() {
  ...
  try {
    ...
    await api.put(`/films/${filmId}`, payload)
    showToast('Film berhasil diupdate!')
    setTimeout(() => router.push('/watchlist'), 1200)
  } catch (err) {
    error.value = err.response?.data?.message || 'Gagal mengupdate film.'
  } finally {
    loading.value = false
  }
}
